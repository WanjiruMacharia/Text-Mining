...
## read one file
Text1 <- readtext("Speeches/May1.pdf")
Text1

## read multiple txt files
Text2 <- readtext("Speeches/*.pdf")
Text2
...
-grep returns returns a vector of indices of the character strings that contains the pattern.
- We will use grep function to remove numbers from our dataset.

```{r}
Names <- c("Faith, Shelmith", "Shelmith2"); Names
grep("Fa",Names)
grepl("Fa", Names)
...
Speeches_tidy[-grep("^[0-9]+", Speeches_tidy$word),] 
```
...
4. Show the most 10 common words on a ggplot chart and a word cloud.
